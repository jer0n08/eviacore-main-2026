          ...
          <div className="mx-auto w-11/12 max-w-6xl">
              ...
              <h2 className="mb-4 font-mono text-3xl md:text-4xl tracking-tight">
                {overview.title.split('\n').map((line, index, lines) => {
                  ...
                  return (
                    <span
                      key={`${line}-${index}`}
                      className={index === 1 ? 'text-amber-300' : undefined}
                    >
                      {line}
                      {index < lines.length - 1 ? <br /> : null}
                    </span>
            ...
            <div className="grid gap-6 lg:grid-cols-3">
              {overview.cards.map((card, index) => (
                ...
                >
                  {index === 0 ? (
                    <ChartCandlestick className="mb-4 h-6 w-6 text-amber-300" aria-hidden="true" />
                  ) : null}
                  {index === 1 ? (
                    <FolderCheck className="mb-4 h-6 w-6 text-amber-300" aria-hidden="true" />
                  ) : null}
                  {index === 2 ? (
                    <div className="loader mb-4" aria-hidden="true" />
                  ) : null}
                  <h3 className="mb-3 font-mono tracking-tight">
                    {card.title}
                  </h3>
